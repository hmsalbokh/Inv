...
1. **الواجهة الأمامية (Vite + React)**:
   ...
   - يمنع تماماً استخدام بادئات مثل `VITE_GEMINI_API_KEY` لمنع تسريب المفتاح للمتصفح أثناء عملية البناء وبث الـ JS.

2. **الخادم السري والمنفذ الآمن (Express Server)**:
   - يستقبل خادم Express البيانات من المتصفح بأمان كامل ويتحقق منها.
   - يقوم الخادم بقراءة متغير البيئة الآمن `GEMINI_API_KEY` أو `API_KEY` مباشرة من ذاكرة النظام السحابي الخادم (Server Runtime Environment).
...
## 🛠️ كيفية إعداد وتفعيل مفتاح البيئة `GEMINI_API_KEY` سحابياً
...
- قم بإضافة متغير بيئة جديد:
  - **Name/اسم المتغير**: `GEMINI_API_KEY`
...
GMAIL_USER=your_smtp_gmail@gmail.com
